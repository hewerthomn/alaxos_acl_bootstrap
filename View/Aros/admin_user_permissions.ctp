...
<div class="row">
    ...
    echo ' ';
    echo $this->Form->end(array('label' =>__d('acl', 'filter'), 'div' => false, 'class'=>'btn'));
    ?>    
    </div>
...
<table class="table table-bordered table-condensed table-striped" >
    <tr>
    	<?php
    	$column_count = 1;    	
    	$headers = array("", $this->Paginator->sort($user_display_field, "Usuário"));
    ...
    foreach($users as $user)
    {
        ...
        echo '<td class="span1">'.$this->Html->link('<i class="icon-lock icon-white"></i>', $link, 
            array('class'=>'btn btn-primary', 'alt' => $title, 'title' => $title, 'escape' => false)).'</td>';
